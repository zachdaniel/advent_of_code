defmodule Aoc2020.Day11 do
  @input "lib/day_11/input.txt"
         |> File.read!()
         |> String.split("\n")
         |> Enum.map(&String.graphemes/1)

  @indexed (for {row, row_num} <- Enum.with_index(@input),
  ...
  def part_one() do
    @indexed
    |> run_to_stabilization(4, {:adjacent, @height, @width})
    |> Map.values()
    |> Enum.count(&(&1 == "#"))
  end
  ...
  def part_two() do
    @indexed
    |> run_to_stabilization(5, :vision)
    |> Map.values()
    |> Enum.count(&(&1 == "#"))
  end

  defp run_to_stabilization(input, adjacency_requirement, adjacency_calc) do
    result =
    ...
    if result == input do
      ...
      run_to_stabilization(result, adjacency_requirement, adjacency_calc)
    end
  end
