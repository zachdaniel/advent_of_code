defmodule Aoc2020.Day20 do
  ...
  def part_one() do
    @input
    |> find_match(@dimensions)
    |> Enum.at(0)
    |> four_corners(@dimensions)
  ...
  def part_two() do
    ...
    {grid, sea_monsters} =
      full_image
      |> orientations(8 * @dimensions - 1)
      |> Enum.map(fn grid ->
        {grid, Enum.filter(grid, &sea_monster_at?(&1, grid))}
      end)
    ...
    Enum.reduce(sea_monsters, grid, fn {x, y}, grid ->
      Enum.reduce(@sea_monster, grid, fn {x_offset, y_offset}, grid ->
  ...
  defp sea_monster_at?({x, y}, grid) do
